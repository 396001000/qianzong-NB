...
const memorySystem = runMemoryScript("audit-memory-system.mjs");
const overlayRoot = path.join(repoRoot, "overlays/qianzong-personal");
const commercialContent = runPackageScript(
  "audit-commercial-content.mjs",
  fs.existsSync(overlayRoot) ? [] : ["--no-require-overlay"]
);
