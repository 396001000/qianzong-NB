...
The repository distributes skills only. It does not distribute `AGENTS.md`, system prompts, runtime prompts, or built-in prompt templates.
...
The default `skills/yonghu-preferences/` profile is commercial-safe. Owner-specific address, assistant-name, persona, and fixed-ending preferences are kept under `overlays/qianzong-personal/` and are applied only when the installer is run with the explicit personal overlay option.
...
- `node scripts/build-commercial-release.mjs --out /tmp/qianzong-NB-commercial --force` creates a release directory without owner-specific overlays.
