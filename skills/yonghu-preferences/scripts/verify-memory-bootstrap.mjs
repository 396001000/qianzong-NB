...
function buildIndex(skillFiles) {
  ...
  const lines = [
    ...
    "",
    "User skills are global user memory and routing context. They live under `$CODEX_HOME/skills/yonghu-preferences/user-skills/`, not inside an individual project.",
    "",
    "## Core Rules",
    "",
  ...
  while (true) {
    for (const marker of markers) {
      if (fs.existsSync(path.join(current, marker))) {
        return { projectRoot: current, marker };
      }
    }
