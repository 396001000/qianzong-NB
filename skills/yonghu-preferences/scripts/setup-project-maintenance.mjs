...
function maintenanceDefaults() {
  return {
    ...
    prePushHook: {
      ...
      command: "node <yonghu-preferences>/scripts/maintain-memory.mjs --scope project --cwd . --trigger pre-push"
    },
    generatedBy: "qianzong-NB memory maintenance",
...
function mergeDefaults(existing) {
  ...
  if (
    ...
    "node <yonghu-preferences>/scripts/maintain-memory.mjs --scope project --cwd . --write-summary --trigger pre-push"
  ) {
...
function buildHookBlock(maintainScript) {
  return `# qianzong-NB memory maintenance start
if command -v node >/dev/null 2>&1; then
  project_root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
  node ${shellQuote(maintainScript)} --scope project --cwd "$project_root" --trigger pre-push
  status=$?
  if [ "$status" -ne 0 ]; then
    echo "qianzong-NB memory maintenance failed. Fix project memory audit errors before pushing." >&2
    exit "$status"
  fi
else
  echo "qianzong-NB memory maintenance skipped: node not found." >&2
fi
# qianzong-NB memory maintenance end`;
}
...
if (!hookPath) {
  ...
  const hasBlock = hookText.includes("qianzong-NB memory maintenance start");
  if (hasBlock && !force) {
    ...
    const cleaned = hasBlock
      ? hookText.replace(
          /# qianzong-NB memory maintenance start[\s\S]*?# qianzong-NB memory maintenance end\n?/m,
          ""
        )
      : hookText;
