...
function isRuntimePromptArtifact(projectRoot, text) {
  const looksLikeSkillsWorkspace =
    hasRel(projectRoot, "skills/yonghu-preferences") &&
    hasRel(projectRoot, "prompts") &&
    (hasRel(projectRoot, "AGENTS5.3.md") || hasRel(projectRoot, "prompts/runtime-v5.2.md"));
...
function hasProjectLocalYonghuPreferences(text) {
  return text.split(/\r?\n/).some((line) => {
    ...
    if (!/(^|[\s`"'(])\.?[\\/]*skills[\\/]+yonghu-preferences/i.test(line)) return false;
...
const forbiddenPatterns = [
  ...
  { name: "project-local-yonghu-preferences", pattern: /(skills[\\/]+yonghu-preferences|user-skills[\\/]|routing-core\.md|memory-reliability-style\.md)/i },
...
for (const item of forbiddenPatterns) {
  ...
  if (item.name === "project-local-yonghu-preferences") {
