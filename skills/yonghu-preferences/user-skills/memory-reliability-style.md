...
{
  "ok": true,
  "globalMemoryRoot": "$HOME/.codex/skills/yonghu-preferences",
...
- Cross-project durable preference -> `yonghu-preferences/references/profile.md` or `yonghu-preferences/user-skills/*.md`.
...
```bash
node "$CODEX_HOME/skills/yonghu-preferences/scripts/verify-memory-bootstrap.mjs" --repair
```
...
```bash
node "$CODEX_HOME/skills/yonghu-preferences/scripts/capture-global-memory.mjs" \
