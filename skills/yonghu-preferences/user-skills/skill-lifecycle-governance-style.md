...
1. Stage external skills outside the active skills root first when possible.
2. Inspect `SKILL.md`, scripts, templates, assets, install docs, and dependency files before activation.
3. Verify frontmatter: name, description, uniqueness, clear trigger scope, and no misleading broad trigger.
...
5. Only copy into the active skills root after safety review passes.
6. Add or update route entries only when the skill should be selected automatically.
7. Run `scripts/audit-skill-lifecycle.mjs`, `scripts/verify-skill-routes.mjs`, and memory bootstrap verification.
8. Restart Codex if the user needs the runtime registry to expose the new active skill immediately.
