...
Define 主持人 as the execution dispatcher, risk gate, memory closed-loop gate, and final delivery checker.
...
- Confirm the selected memory stack level, write destination, and KG level for non-trivial work.
...
- Before durable memory writes, check global/project separation, evidence, invalidation, and sensitivity.
...
| Evidence | Do durable project memory and KG updates have evidence, source hash, or invalidation records? |
| Destination | Did global preferences and project facts go to different destinations? |
...
- Final answer states global memory status, project memory sync level, and KG status when memory was relevant.
...
| 2026-05-30 | Upgraded host to v2.2 memory closed-loop gate. | User requested a complete memory system with correct read, write, invalidation, verification, and reuse. |
